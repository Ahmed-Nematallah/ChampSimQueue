...
void CACHE::handle_fill(bool process_element)
{
	// handle fill
	...
	if (MSHR.next_fill_cycle <= current_core_cycle[fill_cpu]) {
		...
		if (cache_type == IS_LLC) {
			way = llc_find_victim(fill_cpu, MSHR.entry[mshr_index].instr_id, set, block[set], MSHR.entry[mshr_index].ip, MSHR.entry[mshr_index].full_addr, MSHR.entry[mshr_index].type, process_element);
		}
		else
		...
		if ((cache_type == IS_LLC) && (way == LLC_WAY)) { // this is a bypass that does not fill the LLC
			...
			if (cache_type == IS_LLC) {
				llc_update_replacement_state(fill_cpu, set, way, MSHR.entry[mshr_index].full_addr, MSHR.entry[mshr_index].ip, 0, MSHR.entry[mshr_index].type, 0, process_element);
	  ...
	  if (cache_type == IS_L1I)
		...
		cpu = 0;
			...
			// update replacement policy
			if (cache_type == IS_LLC) {
				llc_update_replacement_state(fill_cpu, set, way, MSHR.entry[mshr_index].full_addr, MSHR.entry[mshr_index].ip, block[set][way].full_addr, MSHR.entry[mshr_index].type, 0, process_element);
			}
			else
...
void CACHE::handle_writeback(bool process_element)
{
	// handle write
	...
	if ((WQ.entry[WQ.head].event_cycle <= current_core_cycle[writeback_cpu]) && (WQ.occupancy > 0)) {
		...
		if (way >= 0) { // writeback hit (or RFO hit for L1D)
			...
			if (cache_type == IS_LLC) {
				llc_update_replacement_state(writeback_cpu, set, way, block[set][way].full_addr, WQ.entry[index].ip, 0, WQ.entry[index].type, 1, process_element);
		...
		if(mshr_index == -2)
			...
			if((WQ.entry[index].fill_l1d) && (MSHR.entry[mshr_index].fill_l1d != 1))
			  {
				...
				if (cache_type == IS_LLC) {
					way = llc_find_victim(writeback_cpu, WQ.entry[index].instr_id, set, block[set], WQ.entry[index].ip, WQ.entry[index].full_addr, WQ.entry[index].type, process_element);
				}
				else
		  ...
		  if (cache_type == IS_L1I)
			...
			cpu = 0;
					...
					// update replacement policy
					if (cache_type == IS_LLC) {
						llc_update_replacement_state(writeback_cpu, set, way, WQ.entry[index].full_addr, WQ.entry[index].ip, block[set][way].full_addr, WQ.entry[index].type, 0, process_element);
					}
					else
...
void CACHE::handle_read(bool process_element)
{
	// handle read
	  ...
	  if (read_cpu == NUM_CPUS)
		...
		if (RQ.entry[index].type == LOAD) {
			...
			cpu = 0;
				...
				// update replacement policy
				if (cache_type == IS_LLC) {
					llc_update_replacement_state(read_cpu, set, way, block[set][way].full_addr, RQ.entry[index].ip, 0, RQ.entry[index].type, 1, process_element);
...
void CACHE::handle_prefetch(bool process_element)
{
	// handle prefetch
	  ...
	  if (prefetch_cpu == NUM_CPUS)
		...
		if ((PQ.entry[PQ.head].event_cycle <= current_core_cycle[prefetch_cpu]) && (PQ.occupancy > 0)) {
			...
			if (way >= 0) { // prefetch hit
				...
				// update replacement policy
				if (cache_type == IS_LLC) {
					llc_update_replacement_state(prefetch_cpu, set, way, block[set][way].full_addr, PQ.entry[index].ip, 0, PQ.entry[index].type, 1, process_element);
...
void CACHE::operate()
{
	...
	//uint64_t queue_length = 8;
	//uint64_t queue_occupancy = 0;
	//uint64_t queue_cycle_count = 20;
	
	//uint64_t queue_cycles[queue_length];

	assert(queue_occupancy <= queue_length);

	bool process_element = true;
	bool dropped = false;

	if (cache_type == IS_LLC && implement_queue) {		
		for (uint64_t i = 0; i < queue_length; i++) {
			if (queue_cycles[i] > 0) {
				assert(queue_occupancy > 0);
				queue_cycles[i]--;
				if (queue_cycles[i] == 0) queue_occupancy--;
				if (serial_queue) break;
			}
		}
		if (queue_occupancy >= queue_length) {
			cycles_full_queue++;
			// return;
		...
		// Handle fill
		uint32_t fill_cpu = (MSHR.next_fill_index == MSHR_SIZE) ? NUM_CPUS : MSHR.entry[MSHR.next_fill_index].cpu;
		if (fill_cpu != NUM_CPUS && MSHR.next_fill_cycle <= current_core_cycle[fill_cpu]) {
			uint32_t set = get_set(MSHR.entry[MSHR.next_fill_index].address);
			uint64_t ip = MSHR.entry[MSHR.next_fill_index].ip;
			uint64_t full_Addr = MSHR.entry[MSHR.next_fill_index].full_addr;
			uint32_t queue_type = MSHR.entry[MSHR.next_fill_index].type;
			handle_fill(process_element);
			if (process_element) {
				assert(queue_occupancy < queue_length);
				bool added_to_queue = false;
				for (uint64_t i = 0; i < queue_length; i++) {
					if (queue_cycles[i] == 0) {
						queue_cycles[i] = llc_get_queue_latency(set, 0, ip, full_Addr, queue_type, false, LLC_QUEUE_LATENCY_FILL);
						queue_occupancy++;
						added_to_queue = true;
						break;
					}
				}
				assert(added_to_queue == true);
			} else {
				dropped = true;
				total_operations_dropped++;
			}
		}
		if (queue_occupancy >= queue_length) {
			if (process_element) cycles_full_queue++;
			// return;
			process_element = false;
		}
		...
		// Handle writeback
		uint32_t writeback_cpu = WQ.entry[WQ.head].cpu;
		if (writeback_cpu != NUM_CPUS && (WQ.entry[WQ.head].event_cycle <= current_core_cycle[writeback_cpu]) && (WQ.occupancy > 0)) {
			int index = WQ.head;
			uint32_t set = get_set(WQ.entry[index].address);
			int way = check_hit(&WQ.entry[index]);
			uint64_t full_addr = block[set][way].full_addr;
			uint64_t ip = WQ.entry[index].ip;
			uint32_t queue_type = WQ.entry[index].type;
			handle_writeback(process_element);
			if (process_element) {
				assert(queue_occupancy < queue_length);
				bool added_to_queue = false;
				for (uint64_t i = 0; i < queue_length; i++) {
					if (queue_cycles[i] == 0) {
						queue_cycles[i] = llc_get_queue_latency(set, way, full_addr, ip, queue_type, 1, LLC_QUEUE_LATENCY_WRITE);
						queue_occupancy++;
						added_to_queue = true;
						break;
					}
		...
		// Handle read
		reads_available_this_cycle = (MAX_READ < (queue_length - queue_occupancy)) ? (MAX_READ) : (queue_length - queue_occupancy);
		uint32_t reads_available_this_cycle_before = reads_available_this_cycle;

		uint32_t read_cpu = RQ.entry[RQ.head].cpu;
		if (read_cpu != NUM_CPUS && ((RQ.entry[RQ.head].event_cycle <= current_core_cycle[read_cpu]) && (RQ.occupancy > 0))) {
			// AHMED TODO: This doesn't work well with multicore
			int index = RQ.head;
			uint32_t set = get_set(RQ.entry[index].address);
			int way = check_hit(&RQ.entry[index]);
			uint64_t full_addr = block[set][way].full_addr;
			uint64_t ip = RQ.entry[index].ip;
			uint32_t queue_type = RQ.entry[index].type;
			handle_read(process_element);
			if (process_element) {
				assert(queue_occupancy < queue_length);
				for (uint64_t i = 0; i < reads_available_this_cycle_before - reads_available_this_cycle; i++) {
					bool added_to_queue = false;
					for (uint64_t i = 0; i < queue_length; i++) {
						if (queue_cycles[i] == 0) {
							queue_cycles[i] = llc_get_queue_latency(set, way, full_addr, ip, queue_type, 1, LLC_QUEUE_LATENCY_READ);
							queue_occupancy++;
							added_to_queue = true;
							break;
						}
		...
		// Handle prefetch
		reads_available_this_cycle_before = reads_available_this_cycle;

		uint32_t prefetch_cpu = PQ.entry[PQ.head].cpu;
		// handle the oldest entry
		if (prefetch_cpu != NUM_CPUS && ((PQ.entry[PQ.head].event_cycle <= current_core_cycle[prefetch_cpu]) && (PQ.occupancy > 0))) {
			// AHMED TODO: This doesn't work well with multicore
			int index = PQ.head;
			uint32_t set = get_set(PQ.entry[index].address);
			int way = check_hit(&PQ.entry[index]);
			uint64_t full_addr = block[set][way].full_addr;
			uint64_t ip = PQ.entry[index].ip;
			uint32_t queue_type = PQ.entry[index].type;
			if (PQ.occupancy && (reads_available_this_cycle > 0))
				handle_prefetch(process_element);
			if (process_element) {
				assert(queue_occupancy < queue_length);
				for (uint64_t i = 0; i < reads_available_this_cycle_before - reads_available_this_cycle; i++) {
					bool added_to_queue = false;
					for (uint64_t i = 0; i < queue_length; i++) {
						if (queue_cycles[i] == 0) {
							queue_cycles[i] = llc_get_queue_latency(set, way, full_addr, ip, queue_type, 1, LLC_QUEUE_LATENCY_PREFETCH);
							queue_occupancy++;
							added_to_queue = true;
							break;
						}
		...
		if (dropped) {
			cycles_with_operations_dropped++;
		}

		// if (queue_occupancy > queue_length / 2)
		// 	printf("QO: %d\n", queue_occupancy);
	
		// Find out how many requests were served, make sure they are done one by one
		...
		handle_fill(true);
		handle_writeback(true);
		reads_available_this_cycle = MAX_READ;
		handle_read(true);

		if (PQ.occupancy && (reads_available_this_cycle > 0))
			handle_prefetch(true);
	}
