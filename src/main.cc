...
void print_roi_stats(uint32_t cpu, CACHE *cache)
{
    ...
    if (cache->cache_type == IS_LLC) {
        cout << cache->NAME;
        cout << " CYCLES SPENT WITH QUEUE FULL: " << cache->cycles_full_queue << endl;
        cout << cache->NAME;
        cout << " CYCLES WITH OPERATIONS DROPPED: " << cache->cycles_with_operations_dropped << endl;
        cout << cache->NAME;
        cout << " TOTAL OPERATIONS DROPPED: " << cache->total_operations_dropped << endl;
    }
}
...
int main(int argc, char** argv)
{
    ...
    bool LLC_queue = false;
    bool LLC_serial_queue = false;
    uint64_t LLC_queue_length = 8;
    uint64_t LLC_queue_cycles = 8;
    ...
    while (1) {
        static struct option long_options[] =
        {
            ...
            {"implement_queue",  no_argument, 0, 'q'},
            {"serial_queue",  no_argument, 0, 'a'},
            {"queue_length",  required_argument, 0, 'l'},
            {"queue_cycles",  required_argument, 0, 'y'},
            {0, 0, 0, 0}      
        };

        int option_index = 0;

        c = getopt_long_only(argc, argv, "wihsbqaly", long_options, &option_index);
        ...
        switch(c) {
            ...
            case 'q':
                ...
                break;
            case 'a':
                LLC_serial_queue = true;
                break;
            case 'l':
                LLC_queue_length = atol(optarg);
                break;
            case 'y':
                LLC_queue_cycles = atol(optarg);
                break;
            default:
                abort();
        }
	...
	ooo_cpu[i].L1I.l1i_prefetcher_cache_fill = cpu_l1i_prefetcher_cache_fill;
        ...
        uncore.LLC.implement_queue = LLC_queue;
        uncore.LLC.serial_queue = LLC_serial_queue;
        uncore.LLC.queue_occupancy = 0;
        uncore.LLC.queue_length = LLC_queue_length;
        uncore.LLC.queue_cycle_count = LLC_queue_cycles;
