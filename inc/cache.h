...
#define MAX_LLC_QUEUE_LENGTH 64

#define LLC_QUEUE_LATENCY_READ 0
#define LLC_QUEUE_LATENCY_WRITE 1   // VICTIM
#define LLC_QUEUE_LATENCY_FILL 2   // VICTIM
#define LLC_QUEUE_LATENCY_PREFETCH 3
...
class CACHE : public MEMORY {
  public:
    ...
    uint64_t queue_length = 10;
    uint64_t queue_occupancy = 0;
    uint64_t queue_cycle_count = 50;

    uint64_t cycles_full_queue = 0;
    uint64_t cycles_with_operations_dropped = 0;
    uint64_t total_operations_dropped = 0;

    bool implement_queue = false;
	...
	total_miss_latency = 0;
        ...
        queue_length = 64;
        queue_occupancy = 0;
        queue_cycle_count = 60;
    
        queue_cycles = (uint64_t *)calloc (sizeof (*queue_cycles) * MAX_LLC_QUEUE_LENGTH, 1);
    };
    ...
    void handle_fill(bool process_element),
         handle_writeback(bool process_element),
         handle_read(bool process_element),
         handle_prefetch(bool process_element);
    ...
    void add_mshr(PACKET *packet),
         ...
         llc_update_replacement_state(uint32_t cpu, uint32_t set, uint32_t way, uint64_t full_addr, uint64_t ip, uint64_t victim_addr, uint32_t type, uint8_t hit, bool process_element),
    ...
    uint32_t get_set(uint64_t address),
             ...
             llc_find_victim(uint32_t cpu, uint64_t instr_id, uint32_t set, const BLOCK *current_set, uint64_t ip, uint64_t full_addr, uint32_t type, bool process_element),
             llc_get_queue_latency(uint32_t set, uint32_t way, uint64_t full_addr, uint64_t ip, uint32_t type, uint8_t hit, uint8_t queue_latency_type),
